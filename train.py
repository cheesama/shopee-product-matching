...
if __name__ == '__main__':
    ...
    parser.add_argument('--valid_csv_file', default='data/test.csv')
    parser.add_argument('--valid_root_dir', default='data/test_images')

    args = parser.parse_args()

    # Init model
    embedding_net = ProductFeatureNet(backbone_net=args.backbone_net, feature_dim=args.feature_dim)
    product_encoder = ProductFeatureEncoder(model=embedding_net, lr=args.lr, margin=args.margin)

    # Init DataLoader from Custom Dataset
    train_dataset = ProductPairDataset(csv_file=args.train_csv_file, root_dir=args.train_root_dir)
    train_loader = DataLoader(train_dataset, batch_size=args.batch)
    valid_dataset = ProductPairDataset(csv_file=args.valid_csv_file, root_dir=args.valid_root_dir)
